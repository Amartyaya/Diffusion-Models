...
class Data:
    ...
    def visual_transform_fn(x):
        x = x.permute((1, 2, 0)).clamp(-1.0, 1.0)
        x = 255 * (x + 1) / 2
        return x.to(dtype=torch.uint8, device="cpu").numpy()

    @property
    def visual_transform(self):
        return v2.Lambda(self.visual_transform_fn)
    ...
    def show_img(self, ncols=5, nrows=4):
        ...
        fig.tight_layout()
        vtransform = self.visual_transform
        for i in range(nrows):
            for j in range(ncols):
                ax[i, j].imshow(vtransform(self.dataset[i * ncols + j][0]))
                ax[i, j].set_xticks([])
                ax[i, j].set_yticks([])
        plt.show()
